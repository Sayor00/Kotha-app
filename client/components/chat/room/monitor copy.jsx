...
function Monitor({
  ...
  const renderReplies = (replyTo) => {
    ...
    const pendingReplies = replyTo.filter(replyId => !chats.find(chat => chat._id === replyId));
  
    return pendingReplies.map((replyId) => {
      // Render placeholder for pending reply
      return (
        <div key={replyId} className="relative mb-2 rounded-xl grid grid-cols-[auto_1fr] overflow-hidden bg-black/5 dark:bg-black/20">
          <span className="block w-1 h-full bg-sky-600 dark:bg-sky-200"></span>
          <span className="py-2 px-3">
            Reply can't be loaded because the chat it was replied to is not loaded. Please scroll up to load it
          </span>
        </div>
      );
    }).concat(
            ...
            replyContent = (
              <span>
                {reply.userId === master._id ? (
                  <span className="text-gray-500 dark:text-gray-400">Replying to yourself:</span>
                ) : (
                  <span className="text-gray-500 dark:text-gray-400">Replying to {reply.profile?.fullname}:</span>
                )}
                <br />
                <Linkify as="span">{reply.text}</Linkify>
              </span>
            );
          } else if (reply.file) {
            replyContent = (
  ...
  const handlers = useSwipeable({
    onSwipedLeft: (eventData) => {
      ...
      const chatId = eventData.event.target.getAttribute('data-chat-id');
  ...
  return (
    ...
    >
      ...
      <div id="monitor-content" className="relative py-4 flex flex-col select-none">
        {loadingScroll && (
            ...
            .map((elem, i, arr) => (
              <React.Fragment key={elem._id}>
                ...
                <motion.div
                  ...
                  data-chat-id={elem._id} // Set data attribute for chat ID
                  drag="x" // Enable horizontal dragging
                  dragConstraints={{ left: 0, right: 0 }} // Constrain to horizontal axis
                  dragElastic={0.5} // Add elasticity to dragging motion
                >
                  {selectedChats && (
                    <span
                      className={`${
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <Linkify as="span" className="md:select-text">{elem.text}</Linkify>
                          <span
                            className={`${
